...
const Dashboard = ({style, title}) => {
  ...
  return (<div className="dashboard" style={{...style, minHeight: window.innerHeight + 'px'}}>
          ...
          axisBottom={{
            ...
            renderTick: (tick) => (
              <g transform={`translate(${tick.x},${tick.y})`}>
                ...
                <text
                  text-anchor="end" transform="translate(0,10) rotate(-45)"
                  style={{fontSize: '10px'}}
                >{tick.value}</text>
              </g>
            ),
          ...
          tooltip={({data}) => (
            <div className="tariffs-tooltip">
              ...
              <div className="tariffs-tooltip-pie-block">
                <span className="tariffs-tooltip-title">{data.tariffName}</span>
                <div className="tariffs-tooltip-pie">
                  <ResponsivePie
                    data={[{id: 'ААБ', value: data.aab}, {id: 'НАБ', value: data.nab},]}
                    height={80}
                    width={170}
                    innerRadius={0.65}
                    colors={['#1DBF12', '#E31A1A',]}
                ...
                <div className="tariffs-tooltip-stats">
                  <div className="tariffs-tooltip-stats-inner">
                    <span>ААБ</span>
                    <span>{Number(((data.aab || 0) / ((data.nab + data.aab) || 0) * 100).toFixed(1) || 0)}%</span>
                    <span>{data.aab}</span>
                  </div>
                  <div className="tariffs-tooltip-stats-divider"/>
                  <div className="tariffs-tooltip-stats-inner">
                    <span>НАБ</span>
                    <span>{Number(((data.nab || 0) / ((data.nab + data.aab) || 0) * 100).toFixed(1) || 0)}%</span>
                    <span>{data.nab}</span>
                  </div>
                </div>
              </div>
